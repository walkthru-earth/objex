...
- **COG v0.7.0 workarounds**: See `docs/cog-viewer-architecture.md` for full details. The project tracks `@developmentseed/deck.gl-{raster,geotiff,zarr,epsg,proj} v0.7.0` (bumped from `0.6.1`). `COGLayer` and `MultiCOGLayer` extend `RasterTileLayer` from `@developmentseed/deck.gl-raster`. `COGLayer` exposes `signal?: AbortSignal` as an officially documented prop, threading abort through tile fetches. `MosaicLayer.sources` is reactive to in-place updates by reference — our viewers still bump a content-hashed id (`mosaicId`) on commit, but a future cleanup can drop the id-bump trick once we trust upstream's reactive path under streaming hydration. Non-paletted COGs use linear sampling on the data texture (visual-only, no API impact). Upstream errors on projections with missing `units`, but our `normalizeCrsUnits()` in `utils/cog.ts` runs first and infers `degree`/`meter`/`foot`/`us survey foot` from `projName` / `to_meter`, so we never trip the throw. **0.7.0 migration notes**: (1) deck.gl-raster PR #546 forwards `onTileLoad`/`onTileError`/`onTileUnload`/`onViewportLoad` through `RasterTileLayer` natively, so the entire `deck.gl-raster` callback-pass-through patch was DELETED. (2) `MosaicLayer` was refactored: it now forwards `debounceTime` natively AND renamed its tile callbacks to source-level props — `onSourceLoad`/`onSourceError`/`onSourceUnload(source, { data })` (`source` is the resolved `MosaicSourceMeta`) plus `onViewportLoad`. `StacMosaicViewer` migrated `onTileUnload` → `onSourceUnload` and the MosaicLayer callback hunk was dropped from the geotiff patch. (3) `MinimalTileData` is now exported only from `deck.gl-raster` (not `deck.gl-geotiff`); `utils/cog.ts` imports it from `deck.gl-raster`. (4) Breaking renames `TilesetDescriptor` → `RasterTilesetDescriptor` and `MosaicTileset2D` made private do not affect us (we import neither). The `Colormap` shader module ships `precision highp sampler2DArray;`, so the local `Sampler2DArrayPrecision` shim stays deleted. Remaining workarounds:
  ...
  - Antimeridian longitude wrapping, pnpm patch adds proj4 `+over` flag ([#366](https://github.com/developmentseed/deck.gl-raster/issues/366) and [PR #374](https://github.com/developmentseed/deck.gl-raster/pull/374) both still open in v0.7.0). As of 0.7.0 there is a single patch, `patches/@developmentseed__deck.gl-geotiff@0.7.0.patch`, with three hunks: the proj4 `+over` antimeridian fix in `cog-layer.js`, the `inferRenderPipeline` re-export from index (`index.js` + `index.d.ts`), and the `r16unorm` → `r32float` Firefox/macOS fallback in `MultiCOGLayer.createBandTexture`. The callback-pass-through patch (`deck.gl-raster`) and the MosaicLayer callback/`debounceTime` hunk were both DROPPED because 0.7.0 forwards these natively (deck.gl-raster PR #546 + the MosaicLayer `onSource*` refactor).
  - v0.7.0's `LinearRescale` shader module is wired via `CogControls`. For the library-default uint RGB path, `createRescaledPipeline()` in `utils/cog.ts` wraps `inferRenderPipeline` (re-exported through our pnpm patch) and appends `LinearRescale`. For the custom single-band CPU + GPU Colormap path, `buildCustomRenderTile()` appends `LinearRescale` between `FilterNoDataVal` and `Colormap` so the slider rescales `color.r` before the ramp lookup (no longer hidden in single-band mode). `rescaleApplicable` in `CogViewer` now returns true whenever `bandConfig.mode === 'single'` in addition to the legacy uint-RGB case.
  - v0.7.0's `CutlineBbox` shader module takes prop `{ bbox: [minX, minY, maxX, maxY] }` in mercator meters (EPSG:3857), not lnglat
...
- **wkt-parser `units = "unknown"` gotcha**: wkt-parser sets `def.units` from the WKT `UNIT` node. Some entries in the bundled EPSG CSV have a missing or malformed UNIT node, so `parseWkt()` returns `units: "unknown"`, which makes `geoTiffToDescriptor` throw `Source projection is missing 'units' property`. `normalizeCrsUnits()` in `utils/cog.ts` is applied to every resolver output, inferring `degree` for `longlat`, `meter` for `to_meter === 1` or missing, `foot` for 0.3048, and `us survey foot` for 1200/3937. We always normalize before handing the projection to `COGLayer`, so the upstream throw in 0.7.0 is preempted.

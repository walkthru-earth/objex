...
- **DuckDB-WASM `stoi` bug**: DuckDB-WASM 1.33.1-dev40.0 crashes with `stoi: no conversion` on GeoParquet files with CRS metadata ([duckdb/duckdb-wasm#2199](https://github.com/duckdb/duckdb-wasm/issues/2199), tracked in [walkthru-earth/objex#5](https://github.com/walkthru-earth/objex/issues/5)). TableViewer has error recovery that retries with `enable_geoparquet_conversion = false` and BLOB handling. Also blocks DuckLake support (1.5.x catalogs use storage v68, dev20.0 only reads v64-v67). See `docs/ducklake-wasm-support.md`
...
- **`DETACH IF EXISTS` not in dev20.0**: DuckDB-WASM 1.33.1-dev20.0 does not support `DETACH IF EXISTS`. Use plain `DETACH` wrapped in try/catch

...
| CogViewer | @developmentseed/deck.gl-geotiff v0.6.0-alpha.1, @developmentseed/geotiff | utils/cog, utils/colormap-sprite, utils/url, CogControls. Single-band mode plumbs an `onHistogram` callback through `selectCogPipeline` and stores a 64-bin `Uint32Array` in `$state.raw` for the CogControls histogram overlay. Cleared on tab reset. |
...
| StacMosaicViewer | @developmentseed/deck.gl-geotiff MosaicLayer, @developmentseed/geotiff, DuckDB-WASM (parquet path only) | utils/cog, utils/stac (classifyStac, buildMosaicSourceMeta), utils/stac-hydrate (JSON path), query/stac-geoparquet (stac-geoparquet path → `queryStacGeoparquetFeatureCollection`), utils/url, storage/getAdapter, CogControls, map/MapContainer. Two ingestion paths: (1) progressive JSON link-walking via `hydrateStacItems` (batches of 12), (2) single-shot DuckDB query for `.parquet`/`.geoparquet` extensions that materializes an FC via `stacRowToItem` (from `objex-utils/stac-geoparquet`). Both funnel through `buildMosaicSourceMeta` + `ingestParquetFeatures`/`onBatch` to the same MosaicLayer wiring. Hard cap 2000 items. Band count and sampleFormat are probed dynamically on the first COG that `getSource` resolves (via `geotiff.count` / `cachedTags.sampleFormat`), which reseeds `bandConfig` via `defaultBandConfig(count, sf)` and updates `<CogControls bandCount=...>` so 4-band NAIP-style imagery exposes all four bands in the picker (not the old hardcoded 3). Combined with `needsCustomPipelineForConfig` forcing the CPU path for `count===4`, this fixes the "band 4 becomes alpha" opacity artifact in the library-default RGBA pipeline. Single-band mode also plumbs `onHistogram` into a `$state.raw<Uint32Array>` for the CogControls histogram overlay. Exposes the same `Info` button + pixel-inspection pattern as `CogViewer`: a `geotiffCache: Map<string, Promise<GeoTIFF>>` is populated inside `getSource` and reused both for MosaicLayer rebuilds and for the map-click handler, which finds the topmost source whose bbox contains the click (reverse-iterates `itemsRef` to match mosaic z-order) and calls `readPixelAtLngLat(...)` against that source's GeoTIFF. Info panel shows source count, detected band count + data type (captured as `buildDataTypeLabel(sampleFormat, bitsPerSample)` on the first resolved COG), and union bounds. |
